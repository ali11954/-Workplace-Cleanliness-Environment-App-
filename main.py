...
# تأكد أن مجلد instance موجود (لو غير موجود سينشأ)
os.makedirs(instance_path, exist_ok=True)
app = Flask(__name__)
app.config['SECRET_KEY'] = 'a_very_secret_key_1234567890'  # ضع قيمة سرية وطويلة هنا
app.config['SQLALCHEMY_DATABASE_URI'] = f"sqlite:///{os.path.join(instance_path, 'database.db')}"
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
db.init_app(app)  # ← تهيئة db بالتطبيق فقط
...
if __name__ == "__main__":
    with app.app_context():
        ...
        if not User.query.filter_by(username='admin').first():
            admin = User(fullname='المشرف الرئيسي', username='admin', role='admin', active=True)
